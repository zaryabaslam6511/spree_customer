...
Deface::Override.new(
		...
		name: 'populate_tax_rates_tax_categories_according_to_customer_role',
		replace: 'div[data-hook="category"]',
		text: <<-HTML
					<div data-hook="category">
						<% if defined?(current_spree_customer) && current_spree_customer %>
							<%= f.label :tax_category_id, Spree.t(:tax_category) %>
              <%= f.collection_select(:tax_category_id, current_spree_customer.tax_categories,:id, :name, {}, {class: 'select2'}) %>
						<% else %>
							<%= f.label :tax_category_id, Spree.t(:tax_category) %>
...
Deface::Override.new(
		...
		text: <<-HTML
              ...
              <%= f.collection_select(:zone_id, current_spree_customer.zones, :id, :name, {}, {class: 'select2'}) %>
						<% else %>
							<%= f.label :zone, Spree.t(:zone) %>
