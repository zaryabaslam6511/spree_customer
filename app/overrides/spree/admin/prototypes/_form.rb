Deface::Override.new(
		...
		text: <<-HTML
	            ...
	            <%= f.field_container :property_ids, class: ['form-group'] do %>
					      ...
					      <%= f.collection_select(:property_ids, current_spree_customer.properties, :id, :name_with_presentation, { }, { class: 'select2', multiple: true }) %>
					    <% end %>
						<% else %>
							<%= f.field_container :property_ids, class: ['form-group'] do %>
					      <%= f.label :property_ids, Spree.t(:properties) %><br>
...
Deface::Override.new(
		virtual_path: 'spree/admin/prototypes/_form',
		name: 'populate_option_types_according_to_customer_role',
		...
		text: <<-HTML
	              ...
	              <%= f.collection_select(:option_type_ids, current_spree_customer.option_types, :id, :name_with_presentation, { }, { class: 'select2', multiple: true }) %>
	            <% end %>
						<% else %>
							<%= f.field_container :option_type_ids, class: ['form-group'] do %>
...
Deface::Override.new(
		virtual_path: 'spree/admin/prototypes/_form',
		name: 'populate_taxons_according_to_customer_role',
		replace: 'div#taxons',
		text: <<-HTML
	              ...
	              <%= f.collection_select(:taxon_ids, current_spree_customer.taxons, :id, :name, { }, { class: 'select2', multiple: true }) %>
	            <% end %>
						<% else %>
							<%= f.field_container :taxon_ids, class: ['form-group'] do %>
