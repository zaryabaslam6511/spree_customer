Deface::Override.new(
		...
		name: 'populate_shipping_categories_on_new_product_according_to_customer_role',
		...
		text: <<-HTML
					<div data-hook="new_product_shipping_category" class="col-xs-12 col-md-4">
			          ...
			          <%= f.collection_select :shipping_category_id, current_spree_customer.shipping_categories, :id, :name, {include_blank: Spree.t('match_choices.none')}, {class: 'select2', required: :required} %>
			        <% end %>
						<% else %>
							<%= f.field_container :shipping_category, class: ['form-group'] do %>
			          <%= f.label :shipping_category_id, Spree.t(:shipping_categories) %><span class="required">*</span>
...
Deface::Override.new(
		...
		text: <<-HTML
					<div data-hook="new_product_prototype">
			          ...
			          <%= f.collection_select :prototype_id, current_spree_customer.prototypes, :id, :name, {include_blank: true}, {class: 'select2'} %>
			        <% end %>
						<% else %>
							<%= f.field_container :prototype, class: ['form-group'] do %>
			          <%= f.label :prototype_id, Spree.t(:prototype) %>
