Deface::Override.new(
		...
		name: 'populate_shipping_categories_according_to_customer_role',
		...
		text: <<-HTML
					<div data-hook="admin_product_form_shipping_categories">
			          ...
			          <%= f.collection_select(:shipping_category_id, current_spree_customer.shipping_categories, :id, :name, { include_blank: Spree.t('match_choices.none') }, { class: 'select2' }) %>
...
Deface::Override.new(
		virtual_path: 'spree/admin/products/_form',
		name: 'populate_tax_categories_according_to_customer_role',
		...
		text: <<-HTML
					<div data-hook="admin_product_form_tax_category">
			          ...
			          <%= f.collection_select(:tax_category_id, current_spree_customer.tax_categories, :id, :name, { include_blank: Spree.t('match_choices.none') }, { class: 'select2' }) %>
...
Deface::Override.new(
		...
		text: <<-HTML
					<div data-hook="admin_product_form_stores">
			          ...
			          <%= f.collection_select(:store_ids, current_spree_customer.stores, :id, :name, { include_blank: Spree.t('match_choices.none') }, { class: 'select2' }) %>
			        <% end %>
						<% else %>
							<%= f.field_container :stores, class: ['form-group'] do %>
			          <%= f.label :store_ids, Spree.t(:stores) %>
